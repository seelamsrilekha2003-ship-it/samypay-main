...
export default function DayRetailerSalesReport() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/daily-reports/retailer-sales?date=${date}`);
      if (res.data.success) {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '20px' }}>
          <div>
            <h2 style={{ fontSize: '24px', fontWeight: '800', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '12px', margin: 0 }}>
              <FaUserTie color="#f59e0b" /> Retailer Daily Sales
            </h2>
            <p style={{ color: '#64748b', margin: '4px 0 0 0' }}>Performance summary of all retailers</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' }}>
              <div style={{ background: '#fffbeb', padding: '20px', borderRadius: '16px', border: '1px solid #fcd34d' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                  <FaChartLine color="#d97706" />
                  <div style={{ color: '#d97706', fontSize: '14px', fontWeight: '600' }}>Total Sales Volume</div>
                </div>
                <div style={{ fontSize: '28px', fontWeight: '800', color: '#78350f' }}>₹{(data.summary.total_amount || 0).toLocaleString()}</div>
                <div style={{ fontSize: '13px', color: '#b45309' }}>{data.summary.total_count} transactions</div>
              </div>

              <div style={{ background: '#ecfdf5', padding: '20px', borderRadius: '16px', border: '1px solid #6ee7b7' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                  <FaUsers color="#059669" />
                  <div style={{ color: '#059669', fontSize: '14px', fontWeight: '600' }}>Active Retailers</div>
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              ...
              <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#334155', margin: 0 }}>Top Performing Retailers</h3>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: '#f8fafc' }}>
                  <tr>
                    ...
                    <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700', color: '#64748b' }}>RETAILER NAME</th>
                ...
                <tbody>
                  {data.top_performers.map((user, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '16px', textAlign: 'right', fontWeight: '700', color: '#10b981' }}>₹{user.total_volume.toLocaleString()}</td>
                    </tr>
                  ))}
                  {data.top_performers.length === 0 && (
                    <tr><td colSpan="5" style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No sales recorded for retailers today</td></tr>
                  )}
                </tbody>
              </table>
            </div>
