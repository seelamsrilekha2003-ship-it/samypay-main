...
/* ===============================
   PLANAPI HELPER
================================ */
const callApi = async (endpoint, params, method = "GET", forceHttp = false) => {
...
/* =========================
   🔹 FETCH FASTAG DETAILS (OPTIMIZED)
========================= */
exports.fetchDetails = async (operator, vehicleNumber) => {
  const endpoints = [
    "api/Mobile/FastagInfoFetch", // Primary
    "api/Mobile/FastagFetch",
    "api/Mobile/FastagBillFetch",
    "api/Mobile/BillFetch"
  ];

  const paramVariations = [
    { VehicleNo: vehicleNumber, operator_code: operator, Optional1: "", Optional2: "" }, // Primary
    { vehicle_number: vehicleNumber, operator_code: operator },
    { vehicle_no: vehicleNumber, check_operator: operator },
    { bill_number: vehicleNumber, operator_code: operator },
    { consumer_number: vehicleNumber, operator_code: operator }
  ];

  let lastRes = { STATUS: "FAILED", MESSAGE: "All endpoints failed" };

  // Phase 1: Parallel Probe (Top 1 Var for Top 3 Endpoints) - ~5s total
  console.log(`🚀 FASTAG Phase 1: PARALLEL PROBE...`);
  const phase1Tasks = endpoints.slice(0, 3).map(ep =>
    callApi(ep,
      ep === "api/Mobile/FastagInfoFetch" ? paramVariations[0] : { ...paramVariations[1], operatorcode: operator, Opcode: operator },
      "GET"
    )
  );
  ...
  for (const res of results1) {
    if (res.STATUS === "SUCCESS" || res.STATUS === "1" || (res.DATA && (res.DATA.customername || res.DATA.CustomerName))) {
      return formatResponse(res, vehicleNumber);
    }
  }

  // Phase 2: Sequential Deep Search
  console.log(`🔍 FASTAG Phase 2: DEEP SEARCH...`);
  ...
  return { success: false, message: lastRes.MESSAGE || "Fastag details not found" };
};

const formatResponse = (res, vehicleNumber) => {
  ...
  return {
    ...
    billNumber: data.bill_number || data.BillNo || data.BillNumber || vehicleNumber,
...
exports.processRecharge = async ({
  ...
  const referenceId = `FT${Date.now()}`;

  // ... (Keep existing DB logic for recharge recording) ...
  // Re-implementing simplified DB logic to avoid truncating file issues
  ...
  if (paymentMode === "WALLET") {
    ...
    return { success: true, paymentMode: "WALLET", message: "FASTag bill paid successfully", referenceId, balance: newBalance };
  }
  ...
  if (paymentMode === "RAZORPAY" || paymentMode === "UPI") {
    ...
    return { success: true, paymentMode: paymentMode, message: "FASTag recharge successful (Online)", referenceId };
  }
