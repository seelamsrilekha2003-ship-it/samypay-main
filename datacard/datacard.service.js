...
/* =========================
   🔹 FETCH DATACARD / BROADBAND DETAILS
========================= */
exports.fetchDetails = async (operator, datacardNumber) => {
    const endpoints = [
        "api/Mobile/DatacardBillFetch",
        "api/Mobile/BillFetch",
        "api/Mobile/BroadbandBillFetch", // New for Broadband
        "api/Mobile/DthCustomerInfo"
    ];

    const paramVariations = [
        { bill_number: datacardNumber, operator_code: operator, Optional1: "", Optional2: "" },
        { ConsumerNo: datacardNumber, operator_code: operator }, // Common for BBPS
        { consumer_number: datacardNumber, operator_code: operator },
        { account: datacardNumber, operator_code: operator },
        { number: datacardNumber, operator: operator },
        { phone: datacardNumber, operator_code: operator } // Some use phone
    ];

    let lastRes = { STATUS: "FAILED", MESSAGE: "All endpoints failed" };

    // Phase 1: Parallel Probe (Top 3 Variations for Top 3 Endpoints)
    console.log(`🚀 DATACARD/BROADBAND Phase 1: PARALLEL PROBE...`);
    const phase1Tasks = endpoints.slice(0, 3).map(ep =>
        callApi(ep, { ...paramVariations[0], Opcode: operator }, "GET")
    );
    ...
    for (const res of results1) {
        if (res.STATUS === "SUCCESS" || res.STATUS === "1" || (res.DATA && (res.DATA.customername || res.DATA.CustomerName))) {
            return formatResponse(res, datacardNumber);
        }
    }

    // Phase 2: Sequential Deep Search
    console.log(`🔍 DATACARD/BROADBAND Phase 2: DEEP SEARCH...`);
    ...
    return { success: false, message: lastRes.MESSAGE || "Bill details not found" };
};
...
exports.processRecharge = async ({
    ...
    if (paymentMode === "WALLET") {
        ...
        await db.promise().query(
            ...
            [userId, amount, service, operator, account, `${service} Recharge`, referenceId, currentBalance, newBalance]
        );

        return { success: true, paymentMode: "WALLET", message: "Data Card Recharge successful", referenceId, balance: newBalance };
    }
    ...
    if (paymentMode === "RAZORPAY" || paymentMode === "UPI") {
        ...
        return { success: true, paymentMode: paymentMode, message: "Data Card Recharge successful (Online)", referenceId };
    }
